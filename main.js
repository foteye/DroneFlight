function main() { // Javascript should never really access local files - anything I do to read/support the local .txt file is a hack.
    ...
    for (action of droneInput) {
        ...
        if (action === 'x') { // take photo
            photo = {"x": currentCoords.x, "y": currentCoords.y};

            var unique = true;
            for (unique of uniquePhotos) {
                if (JSON.stringify(photo) == JSON.stringify(unique)) { // This is not a unique photo, break out of this loop.
                    unique = false;
                    break;
                }
            }
