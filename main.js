function main(){
    //Javascript should never really access local files - anything I do to read/support the local .txt file is a hack.
    ...
    var photosTaken = []; // list of snapshots of currentCoords e.g. { 'x' : -3, 'y' : 22};
    var uniquePhotos = []; // list of unique coordinates

    for (action of droneInput){
        if (!action || !(validInput.includes(action))){
            console.log("Action: '", action, "' is not valid input.");
            return;
        }
        if (action === 'x'){
            //take photo
            photosTaken.push({"x" : currentCoords.x, "y": currentCoords.y});
        } else {
            currentCoords = droneAction(action, currentCoords);
            if (currentCoords === false){
                console.log("Error! Unhandled exception in droneAction function");
                continue;
            }
        }
    }

    //Shame that this has to be nsquared - i cant think of another way to find uniqueness.
    for (photo of photosTaken){
        var unique = true;

        for (unique of uniquePhotos){
            if (JSON.stringify(photo) == JSON.stringify(unique)){
                //This is not a unique photo, break out of this loop.
                unique = false;
                break;
            }
        }
...
function droneAction(action, currentCoords){
    if (action === "^"){ //UP
        currentCoords.y += 1;
    } else if (action === "<"){ //Left
        currentCoords.x -= 1;
