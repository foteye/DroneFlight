/* This is my second attempt at the drone flight. The first attempt was O(n^2). I will attempt to finish this one in O(n) time.

    Objective: Using a similar method return an object like the below:

    {
        ...
        x-min: -3
        y-min: -5
        photosTaken: [{ x: 6, y: 1, photos: 2}]
    }

    Initial Thoughts:
    * How in hell is this going to be O(n)?
        - I have to loop through the actions to build my min & max
        - I then have to find out where my photos are taken
        - Can probably do them both at the same time? cant take a photo somewhere you havent been yet ohhhhhhhhhhh so smart

    Lessons Learned:
    * Anytime you get O(n^2) or worse you should look at refactoring
    * Multiple IF's suck, use a switchcase to validate your input, erroring on the default case.
*/
class Drone {
    constructor(droneInput) {
        this.input = droneInput;
        ...
        this.returnObject = {
            ...
            'photosTaken': [{ 'x': 0, 'y': 0, 'photos': 0}]
        };
        this.parseInput();
        console.log(this.returnObject.photosTaken.sort(function(a, b) { 
            return cmp(a.x,b.x) || cmp(a.y,b.y);
        }));
    }

    parseInput() {
        for (var action of this.input) {
            switch (action) {
                case "<": // Left
                    this.moveDrone("x", -1);
                    break;
    ...
    /*  If a new maximum is reached, this MUST be a new set of coordinates as we are only incrementing by 1. This <should> cover
        all of our cartesian plane. This is an incorrect assumption.
        
        What we can assume is that with each new maximum we need to recalculate the bounds of our entire cartesian plane:
        COLUMNS AND ROWS
    */


    moveDrone(axis, delta) {
        // Increment axis
        this.currentCoordinates[axis] += delta;

        // Check if new maximum is reached on axis, update accordingly
        if (this.currentCoordinates[axis] > this.returnObject.max[axis]){
            this.returnObject.max[axis] = this.currentCoordinates[axis];
            console.log("New max " + axis + " || Current: " + JSON.stringify(this.currentCoordinates) + " || Max: " + JSON.stringify(this.returnObject.max) + " | Min :" + JSON.stringify(this.returnObject.min));
            this.updateBounds(axis);
        }

        // Check if new minimum is reached on axis, update accordingly
        if (this.currentCoordinates[axis] < this.returnObject.min[axis]){
            this.returnObject.min[axis] = this.currentCoordinates[axis];
            console.log("New min " + axis + " || Current: " + JSON.stringify(this.currentCoordinates) + " || Max: " + JSON.stringify(this.returnObject.max) + " | Min :" + JSON.stringify(this.returnObject.min));
            this.updateBounds(axis);
    ...
    updateBounds(axis){
        ...
        for (var i = 0; i < (this.returnObject.max[iterAxis] - this.returnObject.min[iterAxis]); i++){
            ...
            orderedCoords.photos = 0;
            this.returnObject.photosTaken.push(orderedCoords);
        }
    }

    //We only take a photo after we have arrived somewhere so it only makes sense 
    takePhoto() { 
