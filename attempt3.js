class Drone {
    ...
    moveDrone(axis, delta) { // Increment axis
    ...
    takePhoto() {
        var coordHash = (this.currentCoordinates.x + "," + this.currentCoordinates.y).toString();

        if (this.returnObject.photosTaken[coordHash]) {
            this.returnObject.photosTaken[coordHash] += 1;
        } else {
            this.returnObject.photosTaken[coordHash] = 1;
        }
    }
...
drone.parseInput();
var uniqueBillboards = Object.keys(drone.returnObject.photosTaken).length;
var totalPhotos = (Object.values(drone.returnObject.photosTaken)).reduce(function(a,b){
    return a + b;
}, 0);
var accuracy = ((uniqueBillboards / totalPhotos) * 100).toFixed(2);


console.log(drone.returnObject);
console.log("There were " + uniqueBillboards + " billboards that were photographed at least once. The total number of photos taken was " + totalPhotos +  ". This means our accuracy on the drone instructions algorithm was " + accuracy + "%.");